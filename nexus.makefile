...
build:
	@if [ -f nexus_build_result.yaml ]; then \
		...
		printf 'language: rust\nbuild_targets:\n  - .\ninstallation_script: "git submodule update --init --recursive && cargo check -p aptos-node && cargo check -p e2e-move-tests"\nrun_test_command: "cargo test -p e2e-move-tests -- poc::test_poc --nocapture"\ndeveloper_note: "LFS files skipped (not needed for audit). Set git config filter.lfs.smudge=cat and filter.lfs.clean=cat if cloning fresh."\nblocking_error: ""\n' > nexus_build_result.yaml; \
	fi
